...
# Scratchpad tests use scratchpad-test-mcp and a single aura instance that
# loads BOTH the orchestration and single-agent scratchpad configs from a
# directory. Tests pick the agent via the `model` field (alias match).
# Has a separate feature flag — NOT in parent `integration` or
