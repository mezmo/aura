...
- **Status line** under the input frame: `[status_line] segments = [...]` in `cli.toml` picks and orders `model`, `cwd`, `git`, `context`, `tokens`, `scratchpad`, `mcp` (all shown by default). Rendered locally from `ui::status_line` — no extra requests or tokens. `context` shows the token count, or a meter/percentage when the agent's `[agent.llm].context_window` is set; it is hidden once a conversation streams orchestration events (no single context to measure).
