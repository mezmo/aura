...
- The web server is the A2A *receiver* (`AURA_ENABLE_A2A=true`); the core crate's `a2a` module is the *sender*: `[a2a.remote.<name>]` entries become one `ask_agent` rig tool registered in `add_all_tools` beside the MCP tools, so single agents and orchestration workers get it (the coordinator sees it in its planning inventory only). The effective `mcp_filter` governs `ask_agent` like an MCP tool
