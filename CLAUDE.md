...
- By-reference arguments: `[mcp.servers.<name>.scratchpad.by_reference]` gives listed fields a `<field>_file` twin, expanded by `ArgReferenceTool` (`scratchpad/arg_reference.rs`), looked up per (server, tool). `add_mcp_tool` wraps it *inside* `config.tool_wrapper` (wrappers/persistence record the reference) unless a `[hitl]` pattern gates the tool, then *outside* (the approver sees the expanded bytes). Raw capture and reference reads are capped at `MAX_RAW_PAYLOAD_BYTES`. The `edit_stored_file` tool (`scratchpad/edit.rs`, named to avoid colliding with MCP servers' own `edit` tools) is registered only when a reachable tool has by_reference fields (`ScratchpadToolsConfig::edit_tool`, decided at scratchpad setup); it writes a new `<name>.edit-<sha256 prefix>.<ext>` file instead of editing in place, and never overwrites a file with different bytes, writing atomically via temp file + hard link (`ScratchpadStorage::write_new_verbatim`). References and edits refuse an intercepted output's rendered file when its `.raw` copy exists. Intercepted results with a single embedded resource also get a verbatim `.raw.<ext>` copy; the payload travels from `extract_tool_result` to `ScratchpadWrapper` via `RawPayloadSlot` (`mcp/response.rs`) → `ToolCallContext::raw_payload`
