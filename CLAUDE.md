...
- Startup banner shows configured orchestration workers and their descriptions; a worker that overrides `[agent.llm]` is annotated with its model. Worker data comes from loaded configs (standalone) or the `/v1/models` `workers` field (HTTP).
