...
use aura_web_server::types::{
    ActiveRequestTracker, AppState, ChatCompletionRequest, ChatMessage, Role,
};

use crate::api::stream::{StreamResult, process_sse_events};
use crate::api::types::{Message, ToolDefinition};
use crate::tools::rig_tools::cli_tools_as_rig_tools;
...
impl DirectBackend {
    ...
    pub async fn from_toml(
        ...
        let app_state = Arc::new(AppState {
            ...
            default_agent: None,
            additional_tools: Arc::new(cli_tools_as_rig_tools),
        });
    ...
    /// Convert CLI messages to web server ChatMessage format.
    fn build_chat_request(messages: &[Message], model: Option<String>) -> ChatCompletionRequest {
        let web_messages: Vec<ChatMessage> = messages
            .iter()
            .map(|m| ChatMessage {
    ...
    pub async fn stream_chat(
        ...
        let mut req = Self::build_chat_request(messages, selected);
        let chat_session_id = format!("cs_{}", uuid::Uuid::new_v4().simple());
    ...
    pub async fn summarize(&self, text: &str) -> Result<(String, Option<(u64, u64)>)> {
        let selected = get_selected_model();
...
mod tests {
    ...
    fn user_role_mapping() {
        ...
        let req = DirectBackend::build_chat_request(&msgs, None);
    ...
    #[test]
    fn unknown_role_mapping() {
        // "tool" role doesn't match any named variant
        let msgs = vec![Message::tool_result("call_1", "Read", "content")];
        let req = DirectBackend::build_chat_request(&msgs, None);
        assert_eq!(req.messages[0].role, Role::Unknown);
    }

    #[test]
    fn none_content_becomes_empty() {
        // assistant_with_tool_calls has content=None when only tool calls
    ...
    fn model_passed_through() {
        ...
        let req = DirectBackend::build_chat_request(&msgs, Some("gpt-4".to_string()));
