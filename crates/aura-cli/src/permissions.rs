...
impl PermissionChecker {
    ...
    pub fn prompt_tool_permission(&mut self, tool_name: &str, arguments: &str) -> bool {
        ...
        eprint!(
            "{}  {} ",
            format!("  Allow {tool_name}?").themed(AuraStyle::Warning),
            "[y]es / [n]o / [a]lways allow / [d]eny always".themed(AuraStyle::Muted),
        );
        ...
        let result = match choice {
            ...
            'd' => {
                ...
                eprintln!(
                    "{}",
                    format!("  ✗ {tool_name}(*) denied for this session.").themed(AuraStyle::Error),
                );
