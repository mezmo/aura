...
/// the user consolidate them with a wildcard in `.aura/settings.json`.
...
impl PermissionChecker {
    /// Load permissions from `.aura/settings.json` in the given directory.
    /// Returns a default checker if the file is missing.
    pub fn load(project_dir: &Path) -> Result<Self> {
        let settings_path = project_dir.join(".aura").join("settings.json");
    ...
    /// hint suggesting the user edit `.aura/settings.json` to use a wildcard.
            ...
            eprintln!(
                ...
                format!(
                    "  Tip: You have {specific_count} path-specific {tool_name} rules in \
                     .aura/settings.json. Consider editing the file to use a wildcard \
    ...
    pub fn prompt_tool_permission(&mut self, tool_name: &str, arguments: &str) -> bool {
        ...
        // Print the chosen key so the user sees what they pressed, then newline.
        ...
        eprintln!("{}", label.with(crossterm::style::Color::White));
        ...
        match choice {
            ...
            'a' => {
                ...
                if let Err(e) = self.persist_rule("allow", tool_name, &allow_pattern) {
                    eprintln!(
                        "{}",
                        format!("  Warning: could not save to settings.json: {e}")
                    ...
                    eprintln!(
                        "{}",
                        format!(
                            "  ✓ Saved {tool_name}({allow_pattern}) to .aura/settings.json — \
            ...
            'd' => {
                ...
                if let Err(e) = self.persist_rule("deny", tool_name, "*") {
                    ...
                    eprintln!(
                        "{}",
                        format!(
                            "  ✗ Saved {tool_name}(*) deny to .aura/settings.json — \
    ...
    /// Persist a permission rule to `.aura/settings.json`.
    /// `kind` must be `"allow"` or `"deny"`.
    fn persist_rule(&self, kind: &str, tool_name: &str, pattern: &str) -> Result<()> {
        let aura_dir = self.project_dir.join(".aura");
        if !aura_dir.exists() {
            std::fs::create_dir_all(&aura_dir)?;
        }

        let settings_path = aura_dir.join("settings.json");
