...
/// Warn at standalone startup when `--enable-client-tools` is set but no
/// loaded config opts in via `[agent].enable_client_tools = true`.
///
/// Returns `Some(message)` to display post-launch when the mismatch is
/// detected, `None` otherwise. The CLI flag advertises tools but the
/// in-process server (which uses the same handler path as `aura-web-server`)
/// only attaches them when the agent's TOML opts in — without this warning
/// the user gets a silent no-op.
#[cfg(feature = "standalone-cli")]
pub fn validate_standalone_client_tools(config: &AppConfig, backend: &Backend) -> Option<String> {
    if !config.enable_client_tools {
        return None;
    }
    let direct = backend.as_direct();
    if direct.any_agent_enables_client_tools() {
        return None;
    }
    Some(
        "--enable-client-tools is set but no loaded config has \
         [agent].enable_client_tools = true. Local tools will not fire. \
         Add `enable_client_tools = true` to the [agent] block of a \
         single-agent TOML config (orchestrated configs are unsupported)."
            .to_string(),
    )
}
