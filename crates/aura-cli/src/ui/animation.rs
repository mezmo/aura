...
fn render_thought_wave(text: &str, wave_pos: f32) -> String {
    let mut result = String::new();
    result.push_str(&format!("{}", "  ⎿ ".themed(AuraStyle::Connector)));
...
/// Render queued input text. Under `--pretty` produces a brightness wave
/// across the characters; otherwise a plain `❯ {text}` styled with the
/// muted role so the queued input is visible without animating.
pub(crate) fn render_queued_wave(text: &str, wave_pos: f32) -> String {
    let mut result = String::new();
    result.push_str(&format!("{} ", "❯".themed(AuraStyle::Prompt)));
    ...
    if !crate::ui::state::is_pretty() {
        result.push_str(&format!("{}", display.themed(AuraStyle::Muted)));
        return result;
    }
...
/// No-op without `--pretty` — the static rendering produced by
/// `render_queued_wave` doesn't need ticking.
                    ...
                    };
                    {
                            ...
                            && !tools.is_empty()
                        {
                            ...
                            for tool in tools.iter() {
                                ...
                                // In-flight pulsing bullet — grey under coloured themes,
                                // terminal-default under no-colors so the pulse stays
                                // visible without re-introducing a tint.
                                let pulsed_grey = if crate::theme::theme().task_palette.is_empty() {
                                    Color::Reset
                                } else {
                                    pulse_color(
                                        Color::Rgb {
                                ...
                                let progress =
                                    tool.progress_message.lock().ok().and_then(|g| g.clone());
                                let line = match progress {
                                    Some(msg) => {
                                        let (tw, _) = term_size();
                                        // Account for `   ⎿ ` prefix (~6 cols) when truncating
                                        let max_len = (tw as usize).saturating_sub(6);
                                        if msg.chars().count() > max_len && max_len > 1 {
                                            let mut s: String =
                                                msg.chars().take(max_len - 1).collect();
                                            s.push('…');
                                            s
                                        } else {
                                            msg
                                    ...
                                    None => format_orch_running(tool.start_time),
                                };
                                print!(
                                    "{}{} {}",
                                    d_prefix.themed(AuraStyle::Connector),
                                    "⎿".themed(AuraStyle::Connector),
                                    line.as_str().themed(AuraStyle::Muted),
                                );
                                let _ = execute!(stdout, cursor::RestorePosition);
                            }
                            drop(last_tool_map);
...
impl ToolStatusAnimation {
    pub fn start(
        ...
        let timer_line = format!(
            "{} {} {}",
            "└─".themed(AuraStyle::Connector),
            "tool started".themed(AuraStyle::Primary),
            format!("({elapsed})").themed(AuraStyle::Primary),
        );
        {
            let _term = lock_term();
    ...
    pub fn finish(mut self, duration: Duration) {
        ...
        println!(
            "{} {} {}",
            "└─".themed(AuraStyle::Connector),
            "tool completed".themed(AuraStyle::Primary),
            format!("({time_str})").themed(AuraStyle::Primary),
        );
        println!();
        redraw_input_frame();
    }
...
pub fn print_tool_call_line(
    ...
    println!(
        ...
        args_display.themed(AuraStyle::Primary),
    );
    println!(
        "{} {}",
        "└─".themed(AuraStyle::Connector),
        "tool requested".themed(AuraStyle::Primary),
    );
    redraw_input_frame();
}
