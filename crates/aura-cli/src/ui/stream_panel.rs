...
fn detail_style_for(event_name: &str) -> Style {
    if event_name.starts_with("aura.") {
        theme().stream_aura
    } else if event_name == "message" {
        theme().stream_message
    } else {
        theme().muted
    }
}
...
pub(crate) fn write_stream_panel_rows_locked(
    ...
    print!("{}", top_border.as_str().themed(AuraStyle::Connector));
    ...
    if let Some(vis_idx) = state.expanded_index {
        ...
        if let Some(evt) = raw_idx.and_then(|ri| state.events.get(ri)) {
            let detail_style = detail_style_for(&evt.event_name);
            let _ = execute!(stdout, cursor::MoveDown(1), cursor::MoveToColumn(0));
            let _ = execute!(stdout, terminal::Clear(terminal::ClearType::CurrentLine));
            ...
            print!("{}", header.as_str().themed_with(detail_style));
            if let Some(ref map) = evt.parsed_data {
                ...
                for line in &detail {
                    ...
                    print!("{}", text.as_str().themed_with(detail_style));
                }
                ...
                print!(
                    "{} {}",
                    "  └─".themed_with(detail_style),
                    display.as_str().themed_with(detail_style),
                );
        ...
        for row in 0..AURA_EVENT_PANEL_MAX_SHOWN {
            ...
            if vis_pos < visible_count {
                ...
                if is_selected {
                    print!("{}", line.as_str().themed(AuraStyle::Selected));
                } else {
                    print!(
                        "{}",
                        line.as_str().themed_with(detail_style_for(&evt.event_name))
                    );
                }
            }
        }
    ...
    let _ = execute!(stdout, cursor::MoveDown(1), cursor::MoveToColumn(0));
    let _ = execute!(stdout, terminal::Clear(terminal::ClearType::CurrentLine));
    print!("{}", bottom_border.as_str().themed(AuraStyle::Connector));
}
...
pub(crate) fn render_stream_panel() {
    if let Ok(state) = STREAM_PANEL.lock() {
        ...
        println!("{}", top_border.as_str().themed(AuraStyle::Connector));
        ...
        if let Some(vis_idx) = state.expanded_index {
            ...
            if let Some(evt) = raw_idx.and_then(|ri| state.events.get(ri)) {
                ...
                println!("{}", header.as_str().themed_with(detail_style));
                if let Some(ref map) = evt.parsed_data {
                    let detail =
                    ...
                    for line in &detail {
                        ...
                        println!("{}", text.as_str().themed_with(detail_style));
                    }
                    for _ in lines_printed..(AURA_EVENT_PANEL_MAX_SHOWN - 1) {
                        println!();
                    }
            ...
            for row in 0..AURA_EVENT_PANEL_MAX_SHOWN {
                ...
                if vis_pos < visible_count {
                    ...
                    if is_selected {
                        println!("{}", line.as_str().themed(AuraStyle::Selected));
                    } else {
                        println!(
                            "{}",
                            line.as_str().themed_with(detail_style_for(&evt.event_name))
