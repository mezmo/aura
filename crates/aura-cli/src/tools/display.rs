...
pub fn print_tool_group(header: &str, display_names: &[String], expanded: bool) {
    ...
    if !display_names.is_empty() {
        ...
        if !deduped.is_empty() {
            ...
            println!(
                ...
                deduped.join(", ").themed(AuraStyle::Muted),
            );
        }
    }
    ...
    if !expanded {
        ...
        println!(
            "{} {}",
            connector.themed(AuraStyle::Connector),
            "... /expand to see more".themed(AuraStyle::Muted),
        );
    }
}
...
pub fn print_tool_call_tree(
    ...
    for (idx, (key, value)) in keys[..show_count].iter().enumerate() {
        ...
        println!(
            "{} {}",
            connector.themed(AuraStyle::Connector),
            key.as_str().themed_with(key_label),
        );
        ...
        for (vi, line) in val_lines.iter().enumerate() {
            ...
            println!(
                ...
                val_connector.themed(AuraStyle::Connector),
    ...
    if has_overflow {
        ...
        println!(
            ...
            format!("... +{remaining} more (/expand)").themed(AuraStyle::Muted),
        );
    }
}
