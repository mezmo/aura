...
pub fn print_update_diff(diff_text: &str, max_diff_lines: usize) {
    ...
    if remaining > 0 {
        println!(
            "{}{}",
            indent,
            format!("... ({remaining} more lines)").themed(AuraStyle::Muted),
        );
    }
}
...
pub fn print_update_commands_summary(commands: &[String], expanded: bool, connector: &str) {
    ...
    println!(
        "{} {}",
        connector.themed(AuraStyle::Connector),
        format!("Using {cmds} to perform updates{suffix}").themed(AuraStyle::Muted),
    );
}
