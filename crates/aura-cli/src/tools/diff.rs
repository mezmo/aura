...
fn compute_diff_system(old_content: &str, new_content: &str) -> Option<(String, usize, usize)> {
    ...
    let old_path = tmp_dir.join("aura_diff_old.tmp");
    let new_path = tmp_dir.join("aura_diff_new.tmp");
...
mod tests {
    ...
    fn compute_diff_identical() {
        // Use fallback directly to avoid temp file contention in parallel tests
        let (_, added, removed) = compute_diff_fallback("hello\n", "hello\n");
        assert_eq!(added, 0);
        assert_eq!(removed, 0);
    }
    ...
    fn compute_diff_system_changes() {
        // Test system diff specifically (serial due to shared temp files)
