...
//!
//! Currently dormant: `cli_tools_as_rig_tools` is not wired into
//! `DirectBackend` because rig's tool loop would execute these tools
//! server-side and bypass the REPL's permission system. See
//! `additional_tools_factory` in `crate::backend::direct` — that is the
//! single place to swap once the client-side-tools protocol lands and both
//! backends can route tool calls back to the REPL for permission gating.
