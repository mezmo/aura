...
impl ToolWrapper for ScratchpadWrapper {
    fn transform_output(
        ...
        // Runs on a tokio worker thread. `block_in_place` signals the
        // scheduler so other tasks can migrate while we do sync IO, instead of
        // starving the runtime.
        let write =
            tokio::task::block_in_place(|| self.storage.write_output_sync(&file_id, &output));
        match write {
            Ok(result) => {
                let filename = result
                    .path
...
mod tests {
    use super::*;
    use crate::scratchpad::context_budget::TokenCounter;
    ...
    async fn test_wrapper_intercepts_at_exact_threshold() {
        ...
        let counter2 = crate::scratchpad::context_budget::TiktokenCounter::default_counter();
    ...
    async fn test_wrapper_falls_back_on_write_failure() {
        ...
        // Should fall back to passing through the original output
        assert_eq!(
            result.output, large_output,
            "On write failure, original output should be returned"
        );
