...
static EMIT_SCRATCHPAD_TOOL_EVENTS: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
    crate::env_flags::bool_env("AURA_EMIT_SCRATCHPAD_TOOL_EVENTS", false)
});

/// Debug toggle: when `AURA_EMIT_SCRATCHPAD_TOOL_EVENTS` is truthy, scratchpad
/// exploration tools surface in the same SSE events as MCP tools
/// (`aura.tool_requested` / `aura.tool_complete` / `aura.tool_usage` and the
/// OpenAI `tool_calls` chunks). Off by default to match orchestration's
/// invisible behavior.
pub fn emit_scratchpad_tool_events_enabled() -> bool {
    *EMIT_SCRATCHPAD_TOOL_EVENTS
}
