...
tokio::task_local! {
    static RUN_ID: Arc<str>;
}

pub fn current_run_id() -> Option<Arc<str>> {
    RUN_ID.try_with(Arc::clone).ok()
}
...
pub async fn with_run_id<F: Future>(run_id: String, f: F) -> F::Output {
    RUN_ID.scope(Arc::from(run_id), f).await
}
...
pub fn scope_stream<S: Stream + Unpin>(run_id: String, inner: S) -> ScopedStream<S> {
    ScopedStream {
...
impl<S: Stream + Unpin> Stream for ScopedStream<S> {
    ...
    /// Entering the scope clones the id, so it is held behind an `Arc` rather
    /// than reallocated on every poll.
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        let this = self.get_mut();
        let run_id = Arc::clone(&this.run_id);
        RUN_ID.sync_scope(run_id, || Pin::new(&mut this.inner).poll_next(cx))
    }
}
