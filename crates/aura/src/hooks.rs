...
impl Deadline {
    ...
    /// Whether the deadline itself has passed, as opposed to the run being
    /// cancelled for some other reason. An unbounded run has none to pass.
...
impl AgentHook for Deadline {
    fn should_cancel(&self) -> bool {
        self.cancelled.is_cancelled() || self.expired()
    }
}
