...
/// The outcome of one worker task. Completion and failure travel as
/// `Ok(TaskOutcome::Completed(..))` / `Err(..)` exactly as before; a worker
/// that parked gated calls (park mode) returns [`TaskOutcome::Blocked`] and
/// the plan records it as `TaskState::AwaitingApproval`.
#[allow(clippy::large_enum_variant)]
enum TaskOutcome {
    Completed(TaskExecutionResult),
    /// The worker's blocked cell held parked calls with a captured snapshot.
    /// The cell drops when `execute_task` returns, so the snapshot and the
    /// blocking attempt travel here — the park commit's checkpoint node
    /// serializes them.
    Blocked {
...
/// Per-attempt park-mode state for one worker stream: the blocked cell the
/// gate appends to and the park-aware hook snapshots into, plus the registry
/// key (used as the hook's request id) the hook looks the cell up by.
struct WorkerPark {
...
pub struct Orchestrator {
    ...
    /// Run-scoped park guard (park mode); `None` off the conversational
    /// route.
...
impl Orchestrator {
    ...
    pub async fn new(
        ...
        // The guard exists only where park mode can register approvals: the
        // conversational route. It stays inert until a worker blocks.
    ...
    ///
    /// `park_cell` arms the gate's park arm for this worker: `Some` only when
    /// park mode is enabled and the route is conversational (see
    /// [`Self::worker_park`]), so the gate parks gated calls into the cell
    /// instead of awaiting a live decision.
    async fn create_worker(
        ...
        if let Some(hitl) = worker_config.hitl.clone() {
            ...
            // Park arm: armed only for workers whose run enabled park mode on
            // the conversational (store-bearing) route. The webhook arm of
            // park mode is out of V1 scope; those deployments keep the live
            // decision path.
    ...
    fn worker_park(&self, task_id: usize, attempt: usize) -> Option<WorkerPark> {
        ...
        Some(WorkerPark {
            ...
            // Worker-stream-scoped synthetic id: unique per (run, task,
            // attempt) and deliberately not the live request id, so the
            // hook's tool-event publishes dead-letter instead of surfacing
            // on the SSE stream.
            key: format!("{}:task:{task_id}:attempt:{attempt}", self.orchestrator_id),
        })
    }

    /// Whether this run parks gated worker calls: `[hitl.park].enabled` set
    /// and the conversational (store-bearing) route. Park mode is default
    /// off; every other configuration keeps the live decision path.
    ...
    ///
    /// `park_key` switches workers in park mode onto the hook-carrying stream
    /// (`stream_chat_with_timeout`) so the park-aware `StreamingRequestHook`
    /// runs: it checks the worker's blocked cell before every LLM turn and
    /// cancels with reason `parked` when a parked call waits. The hook's own
    /// deadline is disabled (`Duration::MAX`) — the orchestrator's per-call
    /// wrap below remains the single wall-clock authority, so the error text
    /// for a timed-out worker is unchanged. The key doubles as the hook's
    /// request id: it is a worker-stream-scoped synthetic id, so the hook's
    /// tool-event publishes dead-letter instead of reaching the live SSE
    /// stream (workers surface tool events through the ObserverWrapper).
...
Assign tasks to the worker whose tools best match the required operations."#,
    ...
    /// across all waves) and the park records for the tasks that blocked —
    /// the park commit's checkpoint nodes serialize them.
    async fn execute(
        &self,
        plan: &mut Plan,
        event_tx: &tokio::sync::mpsc::Sender<Result<StreamItem, StreamError>>,
        ...
        while !plan.is_finished() {
            ...
            if ready_tasks.is_empty() {
                // Quiescence: no ready tasks and none running (waves drain to
                // completion). Park takes precedence over the replan path
                // while a decision is outstanding — the coordinator must not
                // drop a task the human is deciding on.
                match quiescence_verdict(plan) {
                    QuiescenceVerdict::Park => {
                        for line in park_verdict_lines(plan) {
                            tracing::warn!("{line}");
                        }
                    }
                    QuiescenceVerdict::Deadlock => {
                        // No ready tasks but not finished - this shouldn't
                        // happen with valid plans
                        tracing::warn!(
                ...
                futures.next().await
            {
                ...
                match result {
                    ...
                    Ok(TaskOutcome::Blocked {
                        ...
                        // Park mode: the worker parked gated calls and its
                        // captured conversation was confirmed by the cell's
                        // snapshot. The task awaits human decisions; no
                        // artifact, no completion event — one blocked event
                        // per parked call is the wire signal. The snapshot
                        // and attempt are recorded before the cell drops so
                        // the park commit can serialize them.
    ...
    async fn execute_task(
        ...
        for attempt in 1..=MAX_WORKER_ATTEMPTS {
            ...
            // Park mode: a fresh cell per attempt (a normally-failed attempt
            // must not leak entries into its retry) and a per-stream key the
            // park-aware hook looks the cell up by.
            ...
            if let Some(ref park) = park {
                crate::streaming_request_hook::register_park_cell(&park.key, park.cell.clone())
                    .await;
            }
            let mut stream_result = self
                ...
                .await;
            if let Some(ref park) = park {
                crate::streaming_request_hook::remove_park_cell(&park.key).await;
                // The hook keyed its tool-event FIFO under the synthetic
                // stream id; drop any residue the same way request teardown
                // would.
                crate::tool_event_broker::unsubscribe(&park.key).await;
            }

            // The blocked cell is the source of truth after any stream end —
            // including the `Err` the park cancel produces — so it is
            // inspected before the normal result handling.
            if let Some(ref park) = park {
                match park.cell.outcome() {
                    ...
                    CellOutcome::Orphaned { pending } => {
                        // The stream ended before the hook could snapshot
                        // (depth exhaustion or a provider stream error): the
                        // gated action did not run and cannot be resumed from
                        // a conversation that was never captured. Cancel
                        // every pending decision so the human is not left
                        // with a decidable approval nothing will consume,
                        // then fail the task through the normal error path.
    ...
    async fn run_iteration(
        ...
        // ----------------------------------------------------------------
        // PARK PATH (park mode): any awaiting task at quiescence parks the
        // run. The persistence drain above already flushed in-flight tool
        // writes (warn-and-continue; no barrier machinery). The park commit
        // below refreshes the awaiting set against the approval store,
        // publishes the checkpoint document, emits the terminal
        // `run_parked` event, and ends the run — the coordinator must not
        // replan a task the human is deciding on. A failed commit publishes
        // nothing and cancels the run's approvals.
        // ----------------------------------------------------------------
        if quiescence_verdict(&plan) == QuiescenceVerdict::Park {
            tracing::info!(
            ...
            let result = self
                ...
                .await;
            match &result {
                Ok(_) => self.write_parked_manifest(&plan, iteration).await,
                Err(_) => self.write_run_manifest(&plan, iteration, None).await,
            }
    ...
    async fn write_run_manifest(
        ...
        self.write_run_manifest_inner(plan, iterations, None, None, timings)
            .await;
    }

    /// The manifest for a parked run: `RunStatus::Parked` with the awaiting
    /// outcome, so session history records the park instead of deriving a
    /// failure from the incomplete task counts.
    async fn write_parked_manifest(&self, plan: &Plan, iterations: usize) {
        self.write_run_manifest_inner(
            plan,
            iterations,
    ...
    async fn write_run_manifest_with_summary(
        ...
        self.write_run_manifest_inner(plan, iterations, None, response_summary, timings)
            .await;
    }

    async fn write_run_manifest_inner(
        &self,
        plan: &Plan,
        iterations: usize,
...
/// The verdict at the `ready_tasks.is_empty()` quiescence point.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum QuiescenceVerdict {
    /// At least one task awaits a human decision: stop dispatching; park
    /// takes precedence over the replan path.
    Park,
    /// Nothing awaits a decision: the existing replan path (dependency chain
    /// broken by failure).
    Deadlock,
}

/// Whether any task carries non-empty parked calls.
fn quiescence_verdict(plan: &Plan) -> QuiescenceVerdict {
    let awaiting = plan.tasks.iter().any(
        |t| matches!(&t.state, TaskState::AwaitingApproval { pending } if !pending.is_empty()),
    );
    if awaiting {
        QuiescenceVerdict::Park
    } else {
...
mod tests {
    ...
    fn quiescence_single_blocker_mid_dag_parks() {
        ...
        assert_eq!(quiescence_verdict(&plan), QuiescenceVerdict::Park);
    ...
    fn quiescence_without_awaiting_task_is_the_replan_path() {
        ...
        assert_eq!(quiescence_verdict(&plan), QuiescenceVerdict::Deadlock);
    ...
    /// The park commit publishes the document at
    /// `{session_id}/parked/{run_id}.json`, emits the terminal `run_parked`
    /// event, and marks the guard published — the approvals stay parked for
    /// the resume, and the awaiting set re-derives from disk alone.
    #[tokio::test]
    ...
    /// A failing commit publishes nothing, emits no `run_parked` event, and
    /// cancels the run's approvals before the stream ends; the guard's later
    /// drop sweep stays silent instead of double-reporting.
    #[tokio::test]
