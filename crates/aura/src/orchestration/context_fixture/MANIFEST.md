...
| Role-branch append order (vector → scratchpad → skills) | `worker_role_frame_direct` (carries all three appends) - RE-STATED (residual risk R3): the `create_worker` comparison seam proved infeasible without new production code (the preamble capture inside `create_worker` is prompt-journal-gated), so the worker-side append order stays a named residue in DESIGN.md R3 |
| Generic-branch append order (scratchpad → skills) | `worker_generic_fallback` (carries both appends) - RE-STATED (R3), same residue |
