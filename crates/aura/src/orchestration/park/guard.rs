...
/// Cancels a run's parked approvals when the run ends unpublished.
///
/// The orchestrator creates one guard per park-mode run and arms it when a
/// worker returns blocked — the first park-mode registration the run
/// observes. The park commit marks the guard published; any other run end
/// drops it unpublished, and the drop cancels every approval the run parked
/// (with `approval_completed(cancelled)` events), so no decidable approval
/// outlives a run that has no checkpoint. A process crash inside the commit
/// is the one accepted window.
...
impl ParkGuard {
    /// Create the guard for a run. It stays inert until the first
    /// [`record`](Self::record).
    ...
    /// Record the parked calls of one blocked task; the first record arms
    /// the guard.
    pub(crate) fn record(
        &self,
...
mod tests {
    ...
    /// Parked approvals live under the run-scoped owner, so the request
    /// teardown sweep — which fans out by the live request's id — leaves
    /// them parked for the resume.
    #[tokio::test]
    async fn parked_approvals_survive_request_teardown_sweep() {
        let (registry, store) = registry_with_store();
        let decision_id = DecisionId::generate();
        let run_id: RunId = "0191e8c0-1111-7000-8000-000000000042".parse().unwrap();
        ...
        registry.cancel_request("req_live_request").await;

        assert!(
            store.get(&decision_id).await.unwrap().is_some(),
            "the run-scoped owner keeps the parked approval out of the request sweep"
        );
    }

    /// A run that ends without a checkpoint leaves no decidable approval
    /// behind: the unpublished guard's drop cancels the store entries and
    /// publishes one completed(cancelled) event per decision.
    #[tokio::test]
    ...
    /// A published checkpoint disarms the guard: its approvals must survive
    /// the run's end for the resume to consume.
    #[tokio::test]
    ...
    /// A guard that never recorded a decision is inert: nothing to cancel,
    /// no events.
    #[tokio::test]
