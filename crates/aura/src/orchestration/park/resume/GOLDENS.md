...
| Stage 6 (R6): a resumed worker's failure must reach the coordinator loop — it re-plans (a replacement or retried task's worker runs OR a final answer lands) and the run completes; the approved call still executed exactly once | approved-call count + the disjunctive re-plan pin (replacement probe invocation == 1 OR an assistant text turn after the failure report) + completion + cleanup | `resumed_coordinator_replans_when_a_resumed_worker_fails` (aura): FLIPPED GREEN at F2 with only the ruled coordinator-script install — the failure report rides the turns, the scripted respond_directly lands the final-answer leg of the disjunction, completion deletes the checkpoint and clears the store. |
...
- The re-parked segment's turns are the R2 outcome-bearing pair per decided
  ...
  frame and must be reconciled with the panel. Since F2 (the R6
  natural-finish fill) the COMPLETED segment's turns are the run's natural
