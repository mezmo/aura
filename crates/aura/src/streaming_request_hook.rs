...
impl StreamingRequestHook {
    ...
    pub fn new(
        ...
        Self::with_scratchpad_budget(timeout, request_id, None, HashSet::new())
    }
    ...
    pub fn with_scratchpad_budget(
        ...
        let client_tools = Arc::new(ClientTools::new(client_tool_names));
        let hook = Self {
            hooks: Hooks::new()
                .with(deadline.clone())
    ...
    ///
    /// Debug override: when `AURA_EMIT_SCRATCHPAD_TOOL_EVENTS` is truthy,
    /// scratchpad tools are *not* suppressed (caller can observe their
    /// lifecycle in the same SSE events as MCP tools).
    ///
    ...
    pub(crate) fn should_publish_tool_event(tool_name: &str) -> bool {
        if scratchpad::emit_scratchpad_tool_events_enabled() {
            return true;
        }
    ...
    fn check_and_cancel(&self, cancel_sig: CancelSignal, context: &str) {
        ...
        // A blown deadline is an operator's problem; a disconnect is routine.
    ...
    M: CompletionModel,
{
    ...
    fn on_tool_result(
        ...
        async move {
            ...
            self.hooks
                .on_tool_result(&crate::hooks::ToolResult {
                    id: result_id.as_deref(),
                    name: &tool_name,
                })
                .await;
        ...
        M::StreamingResponse: GetTokenUsage,
    {
        ...
        async move {
            if let Some(usage) = usage {
                self.hooks
                    .on_turn_end(crate::hooks::TurnUsage {
