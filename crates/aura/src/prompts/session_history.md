...
- **Learn from failures**: If a prior run failed or produced incomplete results, check its tool chain and failure category to understand what went wrong. Avoid repeating the same approach — try a different decomposition, different tools, or break the task into smaller steps
