...
impl SkillInvocationRecorder {
    /// Build a recorder for one request turn. Every invocation it records
    ...
    /// Persist an invocation before the tool result returns, so a turn that
    /// follows immediately can rehydrate it. The store bounds its own
    /// request latency, and a failure only logs a warning: the turn already
    /// holds the skill content, so losing continuity beats failing the call.
    async fn record(&self, invocation: SkillInvocation) {
