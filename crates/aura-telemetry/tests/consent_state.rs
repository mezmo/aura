...
fn session_event() -> CliSessionStarted {
    CliSessionStarted {
        interactive: true,
...
async fn unknown_holds_and_inspects_would_send() {
    ...
    f.handle.capture(ChatRequestStarted {});
    ...
    assert_eq!(rows[0]["event"], "chat_request_started");
...
async fn enable_does_not_backfill_unknown_events() {
    ...
    // The held chat_request_started must NOT appear on the wire.
    assert!(
        events.iter().all(|e| e["event"] != "chat_request_started"),
