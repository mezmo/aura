...
/// Where the event was emitted from. CLI-only today; the enum is kept
/// (rather than dropped) so the envelope's `aura_source` field stays an
/// explicit, typed value and a future surface can add a variant without
...
/// Coarse deployment-method tag. CLI-only variants: a plain local binary
/// run vs the in-process standalone agent builder (`--standalone`).
...
mod tests {
    ...
    fn deployment_method_parse_known_values() {
        ...
        assert_eq!(
            DeploymentMethod::parse(Some("standalone_cli")),
            DeploymentMethod::StandaloneCli
        );
    }
    ...
    fn property_to_json_is_string_for_enums() {
        ...
        assert_eq!(PropertyValue::Source(Source::Cli).to_json(), json!("cli"));
    }
