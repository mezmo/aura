...
/// concrete defaults live in [`TelemetryConfig::default_for`] for tests
/// and in the `aura-cli` call site for production.
pub struct TelemetryConfig {
    ...
    /// pairs with the 2s shutdown budget aura-cli uses in production.
...
/// block in the per-user `cli.toml`. Every field is optional so partial
/// configs are valid; the bootstrap layer applies env > file > built-in
/// defaults.
///
...
impl FileTelemetryConfig {
    ...
    /// the CLI folds its global + project `cli.toml` configs through it.
...
pub enum EnableOutcome {
    ...
    /// (either a fresh `Unknown → Enabled`, or resuming after a runtime
    /// opt-out).
    Enabled,
...
impl TelemetryHandle {
    ...
    ///
    /// Called by the REPL's first-message consent gate to move `Unknown →
    /// Enabled`. It can also resume after a runtime `set_disabled` (reusing
    /// the sink captured at `init`); that path is covered by tests but is
    /// not reachable from a slash command today, since telemetry is opted
    /// into implicitly by sending a message rather than via a `/telemetry
    /// enable` verb. Either transition is possible only when a sink was
    /// captured at `init` — i.e. init was not a hard `Disabled`. A startup
    /// kill switch or config `enabled = false` leaves `sink_params` empty
    /// and can **never** be resurrected in-process; that returns
    ...
    /// capture never sees `Enabled` with no sender.
    fn ensure_sink(&self) {
    ...
    fn enqueue_or_record(&self, payload: EventPayload) {
        ...
        if let Some(tx) = tx_guard.as_ref() {
            ...
            // On failure `TrySendError::into_inner` hands the payload
            // back, so the happy path moves it without cloning.
            if let Err(e) = tx.try_send(payload) {
                self.inner.dropped.fetch_add(1, Ordering::Relaxed);
                drop(tx_guard);
                self.append_local(&e.into_inner(), false, Some("ChannelFull".into()));
            }
        } else {
    ...
    /// `/telemetry recent` slash command).
...
pub fn init(config: TelemetryConfig) -> TelemetryHandle {
    ...
    // to `enable()`; Disabled never spawns. Matched exhaustively on the
    // state so a future `TelemetryState` variant is a compile error here
    // rather than silently defaulting to "no sink".
    let (sender, bg_handle) = match &state {
        TelemetryState::Enabled => match sink_params.as_ref() {
            Some(p) => {
        ...
        TelemetryState::Unknown | TelemetryState::Disabled(_) => (None, None),
    };
...
async fn flush(ctx: &BackgroundCtx, buf: &mut Vec<Pending>) {
    ...
    // Drain the buffer apart: the wire JSON is sent by move (no clone),
    // and the inspection skeletons are finalized after the POST result
    // is known. `buf` is emptied here either way.
    let (wires, inspecteds): (Vec<Value>, Vec<InspectedEvent>) =
        buf.drain(..).map(|p| (p.wire, p.inspected)).unzip();
    ...
    if let Some(log) = ctx.inspection.as_ref() {
        for mut inspected in inspecteds {
            inspected.sent = sent;
...
mod file_config_merge_tests {
    ...
    fn overlay_wins_for_non_kill_switch_fields() {
        let merged =
            cfg(None, Some("https://base/")).merged_over(cfg(Some(true), Some("https://over/")));
        assert_eq!(
            merged.enabled,
            Some(true),
            "overlay sets enabled when base is silent"
        );
