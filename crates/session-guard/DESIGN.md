...
| `RunDir` / `FencedRun` | A run directory enters the state only via `RunDir::create_under`, which checks the capability at creation (non-recursive; `AlreadyExists` is a hard error) | Run dirs outside admission |
