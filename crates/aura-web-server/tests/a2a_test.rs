...
async fn test_validation_http_a2a_tool_invocation_text_input_only() {
    ...
    let expected_body = json!({
        "error": {
            ...
            "message": "Invalid input: All message parts are expected to be text for this implementation; file and data parts are not supported",
    ...
    assert_eq!(
        ...
        "Error response did not match expected for non-text input"
    );
}
...
async fn test_list_task_get_details() {
    ...
    let _ = tasks
        ...
        .unwrap()
        .iter()
        .find(|t| t.get("id").expect("Task missing 'id' field") == task_id)
        .expect("Created task was not found in list of tasks");
