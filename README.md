...
- [docs/telemetry.md](docs/telemetry.md): notice-gated anonymous product telemetry — the three states (Unknown/Enabled/Disabled), the first-run notice and consent flow, kill switches (including `DO_NOT_TRACK`), full list of what is and isn't collected, install-ID handling, local "would-send" inspection log, self-hosted sink, and the audit guide pointing at the source and test files that implement each guarantee.
